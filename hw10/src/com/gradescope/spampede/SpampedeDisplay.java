...
class SpampedeDisplay {
	...
	public void updateGraphics() {
		...
		// Draw the board
		int gameboardHeight = this.theData.getNumRows();
		int gameboardWidth = this.theData.getNumColumns();
		int gameboardWidthInPixels = gameboardWidth * Preferences.CELL_SIZE;
		
		int horizontalOffset = (this.width - gameboardWidthInPixels) / 2;
		int verticalOffset = 50;
		
		for (int row = 0; row < gameboardHeight; row++) {
			for (int column = 0; column < gameboardWidth; column++) {
				Color cellColor = this.theData.getCellColor(row, column);
				int verticalCellCoord = column*Preferences.CELL_SIZE;
				int horizontalCellCoord = row*Preferences.CELL_SIZE;
				
				this.drawSquare(verticalCellCoord + verticalOffset, horizontalCellCoord + horizontalOffset, cellColor);
			}
		}
