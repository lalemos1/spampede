...
class SpampedeData {
	...
	public void moveSnakeForward(BoardCell cell) {
		changeSnakeDirection(cell);
		
		BoardCell nextCell = getNextCellInDir();
		if (nextCell.isSpam()) {
			makeHead(nextCell);
		} else { // next cell is not spam
			makeHead(nextCell);
			removeTail();
		}
  }
	/**
	 * moveSnakeForward helper function: 
	 * makes the given cell the snake's new head
	 * 
	 * @param nextCell - The next cell which becomes the snake's new head
	 */
	private void makeHead(BoardCell nextCell) {
		BoardCell formerHead = snakeCells.getLast();
		formerHead.becomeBody();
		
		snakeCells.addLast(nextCell);
		nextCell.becomeHead();
	}
	
	/**
	 * moveSnakeForward helper function:
	 * Removes the snake's tail from snake
	 */
	private void removeTail() {
		BoardCell tail = snakeCells.getFirst();
		tail.becomeOpen();
	...
	/**
	 * moveSnakeForward helper function:
	 * Changes the snake's direction to face the cell the snake is moving toward
	 * 
	 * @param cell - the cell which the snake is moving toward
	 */
	private void changeSnakeDirection(BoardCell cell) {
		if (cell == getNextCellInDir()) { // base case: already going correct direction
			return;
		} else if (cell == getNorthNeighbor(cell)) {
			setDirectionNorth();
			return;
		} else if (cell == getSouthNeighbor(cell)) {
			setDirectionSouth();
			return;
		} else if (cell == getEastNeighbor(cell)) {
			setDirectionEast();
			return;
		} else if (cell == getWestNeighbor(cell)) {
			setDirectionWest();
			return;
		} else {
			throw new IllegalArgumentException("Next cell must be cardinally "
											+ "adjacent to the snake head!");
		}
	}

	/**
	 * public BoardCell getNextCellInDir() { switch (this.currentMode) { case
	 * GOING_EAST: return getEastNeighbor(); case GOING_NORTH: return
	 * getNorthNeighbor(); case GOING_SOUTH: return getSouthNeighbor(); case
	 * GOING_WEST: return getWestNeighbor(); default: return null; } }
	 */
	...
	protected BoardCell getNorthNeighbor(BoardCell cell) {
		int northCellRow = cell.getRow() - 1;
		int northCellColumn = cell.getColumn();
		BoardCell northCell = getCell(northCellRow, northCellColumn);
	...
	protected BoardCell getSouthNeighbor(BoardCell cell) {
		int southCellRow = cell.getRow() + 1;
		int southCellColumn = cell.getColumn();
		BoardCell southCell = getCell(southCellRow, southCellColumn);
	...
	protected BoardCell getEastNeighbor(BoardCell cell) {
		int eastCellRow = cell.getRow();
		int eastCellColumn = cell.getColumn() + 1;
		BoardCell eastCell = getCell(eastCellRow, eastCellColumn);
	...
	protected BoardCell getWestNeighbor(BoardCell cell) {
		int westCellRow = cell.getRow();
		int westCellColumn = cell.getColumn() - 1;
		BoardCell westCell = getCell(westCellRow, westCellColumn);
